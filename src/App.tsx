...
export default function App() {
  ...
  useEffect(() => {
    ...
    setConfig((prev) => ({ ...prev, power: wantsRedundant ? 'Redundant' : prev.power }));
  }, [config.accessories]);
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 960, margin: '0 auto', padding: 24 }}>
        <h1 style={{ fontSize: 28, fontWeight: 600 }}>Konfigurátor časových serverů Elproma NTS</h1>
        <p style={{ color: '#555', fontSize: 14 }}>Průvodce: počet zařízení → požadovaná přesnost → doplňky → export.</p>
        ...
        {step === 0 && (
          <section style={box}>
            ...
            <div style={{ padding:'0 24px 24px', display:'flex', justifyContent:'flex-end' }}>
              <button onClick={() => setStep(1)} className="btn-primary">Pokračovat</button>
            </div>
          </section>
        )}
        ...
        {step === 1 && (
          <section style={box}>
            ...
            <div style={{ padding: 24, display:'grid', gap:12, gridTemplateColumns:'1fr 1fr' }}>
              <div>
                {ACCURACY_LEVELS.map((a) => (
                  <label key={a.id}
                    style={{ display:'flex', gap:12, alignItems:'start', padding:12, border:'1px solid #e5e5e5', borderRadius:12, cursor:'pointer', marginBottom:8 }}>
            ...
            <div style={{ padding:'0 24px 24px', display:'flex', justifyContent:'space-between' }}>
              <button onClick={() => setStep(0)} className="btn-ghost">Zpět</button>
              <button onClick={() => setStep(2)} className="btn-primary">Pokračovat</button>
            </div>
          </section>
        )}
        ...
        {step === 2 && (
          <section style={box}>
            ...
            <div style={{ padding:'0 24px 24px', display:'flex', justifyContent:'space-between' }}>
              <button onClick={() => setStep(1)} className="btn-ghost">Zpět</button>
              <button onClick={() => setStep(3)} className="btn-primary">Pokračovat</button>
            </div>
          </section>
        )}
        ...
        {step === 3 && (
          <section style={box}>
                ...
                <div style={{ display:'flex', gap:8, marginBottom:8 }}>
                  ...
                  <button
                    onClick={() => {
                      ...
                      a.href = url; a.download = recommendedModel.id + '-konfigurace.json';
            ...
            <div style={{ padding:'0 24px 24px', display:'flex', justifyContent:'space-between' }}>
              <button onClick={() => setStep(2)} className="btn-ghost">Zpět</button>
              <button
                onClick={() => {
                  setStep(0);
        ...
        <div style={{ marginTop:24, fontSize:12, color:'#666' }}>
          © {new Date().getFullYear()} Prototyp konfigurátoru – rozhodovací průvodce (interní testování).
        </div>
      </div>
    </div>
  );
